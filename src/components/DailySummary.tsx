...
export default function DailySummary() {
  ...
  const projectBreakdown = state.projects.map((project) => {
    ...
    const completedTasks = completedToday.filter((t) => t.projectId === project.id);
  ...
  const getMomentumLevel = (score: number) => {
    if (score >= 80) return { label: 'On Fire!', color: 'text-orange-400', icon: Flame };
    if (score >= 60) return { label: 'Strong', color: 'text-green-400', icon: TrendingUp };
    if (score >= 40) return { label: 'Building', color: 'text-yellow-400', icon: Target };
    return { label: 'Starting', color: 'text-slate-400', icon: Target };
  };
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="bg-gradient-to-br from-green-500/10 to-green-500/5 border border-green-500/20 rounded-2xl p-6">
          ...
          <p className="text-sm text-slate-400">tasks finished today</p>
        </div>

        {/* Focus Time */}
        <div className="bg-gradient-to-br from-indigo-500/10 to-indigo-500/5 border border-indigo-500/20 rounded-2xl p-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-12 h-12 bg-indigo-500/20 rounded-xl flex items-center justify-center">
              <Clock className="text-indigo-400" size={24} />
            </div>
            <div>
              <p className="text-sm text-indigo-400/80">Deep Work</p>
              <p className="text-3xl font-bold text-white">{focusMinutes}m</p>
            </div>
          </div>
          <p className="text-sm text-slate-400">of focused time</p>
        </div>
        ...
        <div className="bg-gradient-to-br from-orange-500/10 to-orange-500/5 border border-orange-500/20 rounded-2xl p-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-12 h-12 bg-orange-500/20 rounded-xl flex items-center justify-center">
              <MomentumIcon className="text-orange-400" size={24} />
            </div>
            <div>
              <p className="text-sm text-orange-400/80">Momentum</p>
      ...
      <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-white">Strategic Momentum</h3>
      ...
      {completedToday.length === 0 && focusMinutes === 0 && (
        <div className="bg-slate-800/30 border border-slate-700 border-dashed rounded-xl p-12 text-center">
          ...
          <h3 className="text-white font-medium mb-2">Ready to earn your receipt?</h3>
