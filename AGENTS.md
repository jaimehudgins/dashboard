...
Slack channel mention tasks use `leo-slack-mentions.sql`, the server-only Leo service role, and signed `app_mention` events from `SLACK_ALERT_USER_ID` only. See `SLACK-MENTION-TASKS.md`. Keep thread evidence untrusted, the extractor tool-free, task writes bounded to one pending task, and acknowledgements private even when alerts target a shared channel. Preserve message/event deduplication; never test by creating real Slack messages/tasks automatically. Run `node scripts/check-slack-mentions.mjs` offline.

Response-needed assessments and partner-specific correction examples require `partner-response-policy.sql` after the two partner-response migrations. The five-minute mail cron assesses a bounded backlog independently of the four draft windows; preserve the separate lease, latest-message/version checks, and human-only closure. `node scripts/check-response-policy.mjs` verifies this offline. Reusable email-type rules require the additive `partner-response-rules.sql` migration and separate explicit user approval; partner exceptions override the same global email type and must never leak to other partners. `node scripts/check-response-rules.mjs` verifies approval/isolation offline. Never automatically promote feedback to a global rule or create tasks/send emails during assessment.
