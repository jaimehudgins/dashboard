...
assert.equal(alreadySent.item().status, "needs_input", "an outgoing message requires assessment, not automatic Waiting");
assert.equal(alreadySent.drafts(), 0, "do not draft a second reply when the latest message is already sent");
assert.equal(alreadySent.batch().needs_input, 1);
